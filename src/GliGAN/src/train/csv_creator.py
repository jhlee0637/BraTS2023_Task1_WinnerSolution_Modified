...
def get_training_dict(args, datadir):
    ...
    # Handle BraTS2021 nested directory structure
    for dataset_category in os.listdir(datadir):
        dataset_category_path = os.path.join(datadir, dataset_category)
        if not os.path.isdir(dataset_category_path):
            continue
            
        for sub_dir in os.listdir(dataset_category_path):
            sub_dir_path = os.path.join(dataset_category_path, sub_dir)
