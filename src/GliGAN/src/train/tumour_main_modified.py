'''
    Modified by Jehee Lee (https://github.com/jhlee0637/)
        1) Added parameter 'outputdir'
        2) Added parameter 'epoch_saving'
        3) Modified function 'load_dict'
            * 'model_pth' is now combined with 'outputdir'
        4) Modified function 'get_nets'
            * img_size=(96, 96, 96)
        5) Modified Saving Checkpoint
            * you can use the parameter which epoch you are going to save weights
...
def load_dict(args, generator, G_optimizer, discriminator, D_optimizer):
        ...
        #model_pth = f"../../Checkpoint/{args.logdir}" # deleted
        model_pth = f"{args.outputdir}/{args.logdir}" # Path to the checkpoint folder 
...
def train(args, global_step, train_loader, generator, G_optimizer, recon_loss, w_progression, discriminator, D_optimizer, HOME_DIR, real_label=None, fake_label=None, criterion=None):
    ...
    for step, batch in enumerate(train_loader):
        ...
        for D_n_i in range(args.D_n_update):
            ...
            x_crop_pad = batch["scan_t1ce_crop_pad"][0:args.batch_size].to(device) # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch["scan_t1ce_noisy"][0:args.batch_size].to(device) # Scan cropped, with padding to 96x96x96 and noise
            y_crop_pad = batch["label_crop_pad"][0:args.batch_size].to(device) # Label cropped and with padding to 96x96x96
            ...
            if args.not_abs_value_loss=="True":
                ...
                label_real = torch.full(size=(args.batch_size,), fill_value=real_label, dtype=torch.float).to(device)
                loss_real = criterion(out_real, label_real)
                ########### Fake ###########
                out_fake = discriminator(input_fake).view(-1) 
                label_fake = torch.full(size=(args.batch_size,), fill_value=fake_label, dtype=torch.float).to(device)
        ...
        for G_n_i in range(args.G_n_update):
            #batch = next(iter(train_loader))
            # Get data from the batch
            x_crop_pad = batch["scan_t1ce_crop_pad"][args.batch_size:].to(device) # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch["scan_t1ce_noisy"][args.batch_size:].to(device) # Scan cropped, with padding to 96x96x96 and noise
            y_crop_pad = batch["label_crop_pad"][args.batch_size:].to(device) # Label cropped and with padding to 96x96x96
            ...
            if args.not_abs_value_loss=="True":
                out_fake = discriminator(input_fake).view(-1) 
                label_real_2 = torch.full(size=(args.batch_size,), fill_value=real_label, dtype=torch.float).to(device)
...
def __main__():
    ...
    parser.add_argument("--outputdir", default="../../Checkpoint", type=str, help="Checkpoint had to be created from the previous process with csv_creator") # added
    ...
    parser.add_argument("--epoch_saving", type=int, default=10, help="save parameters every epoch with this value") # added
    args = parser.parse_args() 

    #HOME_DIR = f"../../Checkpoint/{args.logdir}" # deleted
    HOME_DIR = f"{args.outputdir}/{args.logdir}" # added
    ...
    while global_step < args.num_steps:
        ...
        ### Saving checkpoint ###
        #if (epoch%10==0) or (global_step+1>=args.num_steps): # TODO change to 1000 # deleted
        if (epoch%args.epoch_saving==0) or (global_step+1>=args.num_steps):
