'''
    Modified by Jehee Lee (https://github.com/jhlee0637/)
        ...
        2) 'HOME_DIR' is now combined with the paremeters 'outputdir' and 'logdir' 
        3) Modified function 'get_training_dict'
            * removed one level of nested directory looping.
'''
    ...
    for sub_dir in os.listdir(datadir):
        ...
        for file in os.listdir(sub_dir_path):
            if file.endswith("seg.nii.gz") or file.endswith(args.seg_ending):
                label = os.path.join(sub_dir_path, file)
            else:
                images.append(os.path.join(sub_dir_path, file))
        
        # Only add entry if label was found
...
def __main__():
    ...
    parser.add_argument("--outputdir", default="../../Checkpoint", type=str, help="Directory to save all experiments") # added 
    ...
    HOME_DIR = os.path.join(args.outputdir, args.logdir) # added
    ...
    if args.csv_path == "":
        #CSV_PATH = f'../../Checkpoint/{args.logdir}/{args.logdir}.csv' # deleted
        CSV_PATH = f'{HOME_DIR}/{args.logdir}.csv' # added
    else:
